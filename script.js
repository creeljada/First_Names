...
async function overlayNativeLandBoundariesForParks(features, targetMap) {
	...
	for (let index = 0; index < features.length; index += 1) {
		...
		const parkName = getFeatureParkName(feature, "Unknown park");
...
async function loadParkGeodata() {
	let jsonError = null;

	try {
		const json = await loadJson(GEOJSON_PATH);
		const jsonFeatures = normalizeFeatures(json);
		if (jsonFeatures.length) {
			return json;
		}
	} catch (error) {
		jsonError = error;
	...
	if (typeof window.shp !== "function") {
		if (jsonError) {
			throw new Error(`Failed to load ${GEOJSON_PATH}: ${jsonError.message}`);
		}
		throw new Error(`No features found in ${GEOJSON_PATH}.`);
	}

	try {
		return await window.shp(SHAPEFILE_PATH);
	} catch (shapeError) {
		const jsonMessage = jsonError ? ` GeoJSON error: ${jsonError.message}` : "";
		throw new Error(`Failed to load park geometry from ${SHAPEFILE_PATH}.${jsonMessage} Shapefile error: ${shapeError.message}`);
	}
}
...
function realignFeatureGeometriesByKnownCentroids(features) {
	...
	return features.map((feature) => {
		const parkName = getFeatureParkName(feature, "");
...
function getFeatureParkName(feature, fallback = "") {
	return String(
...
function normalizeFeatures(geojson) {
	...
	if (Array.isArray(geojson)) {
		...
		return geojson.flatMap((entry) => normalizeFeatures(entry));
	}
...
function drawParks(features, targetMap, styleOptions) {
	...
	const layer = L.geoJSON(collection, {
		...
		onEachFeature: (feature, featureLayer) => {
			const name = getFeatureParkName(feature, "Unknown park");
...
function renderSummary(features, selectedNativeLand) {
	...
	const items = features
		.map((feature, index) => {
			const name = getFeatureParkName(feature, `Park ${index + 1}`);
